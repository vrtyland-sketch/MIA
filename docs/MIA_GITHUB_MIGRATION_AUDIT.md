...
| **Private repo** | **NO** — still Public (manual step) |
| **Preflight tests** | **OK** (155/155 fast) |
| **Engine 2.0 doc** | Created — see §7 |
...
| SSH test | **FAIL** — `Permission denied (publickey)` |
...
`.gitignore` entries (lines 38–41) enforce the same for future commits.
...
| `npm run test:preflight:fast` | 0 | **PASS** — 155 passed, 0 failed |
...
|---|---------|-------|--------|
| 1 | Repo still **Public** | Human | GitHub ? Settings ? Danger zone ? Change visibility ? Private |
...
1. **Set repo Private** (Settings ? Danger zone).
2. Optionally add SSH key and `git remote set-url origin git@github.com:vrtyland-sketch/MIA.git`.
3. Continue **graphics / Koj** sprint — Engine 2.0 implementation deferred.
4. When ready for canon import: separate commit for `shared/mia-*-core/` + contract tests (not mixed with live `data/`).
5. Tag **`v0.1-stream-core`** is live — use as rollback checkpoint before Engine 2.0 module extraction.
